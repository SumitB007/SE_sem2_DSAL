...
def accept_set(A,Str): #Accepting the elements and appending to make the set
...
def find_intersection_set(A,B,C): #Intersection of sets
   for i in range(len(A)): 
      flag = search_set(B,A[i])
      if(flag == 1) :
          C.append(A[i])

def find_difference_set(A,B,C): #Difference that is students who play only one game
    for i in range(len(A)): 
      flag = search_set(B,A[i])
      if(flag == 0) :
          C.append(A[i])    
...
def find_union_set(A,B,C): #Union of sets
   for i in range(len(A)):
      C.append(A[i])
